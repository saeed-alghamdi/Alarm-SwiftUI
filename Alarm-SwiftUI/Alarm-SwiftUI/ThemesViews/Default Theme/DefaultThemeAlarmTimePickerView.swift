...
struct DefaultThemeAlarmTimePickerView: View {
        ...
        NavigationView {
            ...
            ZStack {
                ...
                VStack {
                    ...
                    DatePicker("", selection: $currentTime, displayedComponents: .hourAndMinute).colorInvert().colorMultiply(.white).datePickerStyle(.wheel).offset(x: -UIScreen.main.bounds.width / 4, y: 0).shadow(color: .blue, radius: 1, x: 0, y: 0).shadow(color: .blue, radius: 1, x: 0, y: 0)
                    
                    Text(currentTime.formatted(date: .omitted, time: .shortened)).foregroundColor(.white).font(.title).foregroundColor(.blue).bold().padding(.bottom, 20)
                    ...
                    VStack(spacing: 20) {
                        ...
                        Button {
                            ...
                            RoundedRectangle(cornerRadius: 20).strokeBorder(.white, lineWidth: 3.0).foregroundColor(.white).shadow(color: .blue, radius: 2, x: 0, y: 0).shadow(color: .blue, radius: 2, x: 0, y: 0).shadow(color: .blue, radius: 2, x: 0, y: 0).shadow(color: .blue, radius: 2, x: 0, y: 0).frame(width: UIScreen.main.bounds.width / 1.9, height: UIScreen.main.bounds.height / 9, alignment: .center).overlay {
                                
                                Text("Set Alarm").foregroundColor(.white).shadow(color: .blue, radius: 2, x: 0, y: 0).shadow(color: .blue, radius: 2, x: 0, y: 0)
...
struct SecondView_Previews: PreviewProvider {
    static var previews: some View {
        DefaultThemeAlarmTimePickerView(isSetAlarm: .constant(false), alarmTime: .constant("4:30"), dateCompo: .constant(Calendar.current.dateComponents([.hour, .minute, .second], from: Date.now)))
    }
}
