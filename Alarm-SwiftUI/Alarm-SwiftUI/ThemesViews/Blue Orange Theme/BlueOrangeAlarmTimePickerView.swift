//
//  BlueOrangeAlarmTimePickerView.swift
//  Alarm-SwiftUI
//
...
struct BlueOrangeAlarmTimePickerView: View {
        ...
        NavigationView {
            ...
            ZStack {
                ...
                VStack {
                    ...
                    DatePicker("", selection: $currentTime, displayedComponents: .hourAndMinute).datePickerStyle(.wheel).offset(x: -UIScreen.main.bounds.width / 4, y: 0).shadow(color: .orange, radius: 1, x: 0, y: 0).shadow(color: .orange, radius: 1, x: 0, y: 0)
                    
                    Text(currentTime.formatted(date: .omitted, time: .shortened)).foregroundColor(.orange).font(.title).foregroundColor(.orange).bold().padding(.bottom, 20)
                    ...
                    VStack(spacing: 20) {
                        ...
                        Button {
                            ...
                            RoundedRectangle(cornerRadius: 20).strokeBorder(.orange, lineWidth: 3.0).foregroundColor(.orange).shadow(color: .orange, radius: 2, x: 0, y: 0).shadow(color: .orange, radius: 2, x: 0, y: 0).shadow(color: .orange, radius: 2, x: 0, y: 0).shadow(color: .orange, radius: 2, x: 0, y: 0).frame(width: UIScreen.main.bounds.width / 1.9, height: UIScreen.main.bounds.height / 9, alignment: .center).overlay {
                                
                                Text("Set Alarm").foregroundColor(.orange).shadow(color: .orange, radius: 2, x: 0, y: 0)//.shadow(color: .orange, radius: 2, x: 0, y: 0)
...
struct BlueOrangeAlarmTimePickerView_Previews: PreviewProvider {
    static var previews: some View {
        BlueOrangeAlarmTimePickerView(isSetAlarm: .constant(false), alarmTime: .constant("4:30"), dateCompo: .constant(Calendar.current.dateComponents([.hour, .minute, .second], from: Date.now)))
    }
}
